...
describe("deployment-signed Host legacy principal migration", () => {
	...
	it("rejects a caller-owned ancestor even when only its owner can write it", async () => {
		const env = await fixture();
		const ancestor = dirname(
			dirname(await env.filesystem.realpath(env.manifestPath)),
		);
		const originalLstat = env.filesystem.lstat;
		env.filesystem.lstat = async (path) => {
			const info = await originalLstat(path);
			return path === ancestor
				? Object.assign(info, {
						uid: 1000,
						mode: (info.mode & ~0o7777) | 0o700,
					})
				: info;
		};
