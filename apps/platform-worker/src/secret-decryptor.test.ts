	...
	it("decrypts with the source key and re-encrypts with fresh target material", async () => {
		...
		const decision = await crypto.reencrypt({
			...
			expectedBinding: {
				...
				wrappingKeyVersion: sourceRecord.crypto.wrappingKeyVersion,
				dekFingerprint: sourceRecord.crypto.dekFingerprint,
			},
		...
		await expect(
			missingKeyCrypto.reencrypt({
				...
				expectedBinding: {
					...
					dekFingerprint: fixture.record.crypto.dekFingerprint,
				},
				targetKeyVersion: "key_target",
				traceId: "trace_rotation_01",
			}),
		...
		await expect(
			crypto.reencrypt({
				...
				expectedBinding: {
					...
					ownerId: fixture.record.ownerId,
					name: fixture.record.name,
					wrappingKeyVersion: "other_key",
					dekFingerprint: "0".repeat(64),
				},
				targetKeyVersion: "key_target",
				traceId: "trace_rotation_01",
			}),
