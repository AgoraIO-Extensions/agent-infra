...
describe("deployment Secret preparation", () => {
	...
	it("rejects duplicate names and repeated Core names or IDs", async () => {
		await expect(
			prepare.prepareConfigurationSecrets({
				...metadata,
				configurationRevision: 6,
				identityAuthorizationRevision: identity.authorizationRevision,
				secrets: [
	...
	it("does not retry encryption after the encryptor fails", async () => {
		let calls = 0;
		const failing = createDeploymentSecretPreparation({
			encrypt() {
				calls++;
				throw new Error("controlled encryption failure");
			},
		});
		const { attachment } = await failing.prepareConfigurationSecrets({
			...metadata,
			configurationRevision: 6,
			identityAuthorizationRevision: identity.authorizationRevision,
			secrets: [],
		...
		await expect(
			attachment?.resolve({ schemaVersion: 1, expected: [expectation] }),
		).rejects.toThrow("controlled encryption failure");
		await expect(
			attachment?.resolve({ schemaVersion: 1, expected: [expectation] }),
		).rejects.toThrow("spent");
		expect(calls).toBe(1);
