...
CREATE TYPE "platform"."outbox_status" AS ENUM ('pending', 'processing', 'retry_scheduled', 'succeeded', 'failed');
--> statement-breakpoint
CREATE TYPE "platform"."audit_outcome" AS ENUM ('succeeded', 'rejected', 'failed');
--> statement-breakpoint
CREATE TYPE "platform"."idempotency_status" AS ENUM ('reserved', 'completed');
--> statement-breakpoint
...
--> statement-breakpoint
CREATE INDEX "outbox_eligibility_idx" ON "platform"."outbox_items" USING btree ("status", "available_at");
--> statement-breakpoint
...
--> statement-breakpoint
CREATE INDEX "audit_trace_idx" ON "platform"."audit_events" USING btree ("trace_id");
--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX "idempotency_scope_key_unique" ON "platform"."idempotency_records" USING btree ("scope_type", "scope_id", "actor_id", "command_type", "idempotency_key");
--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX "persisted_event_stream_sequence_unique" ON "platform"."persisted_events" USING btree ("stream_id", "sequence");
--> statement-breakpoint
