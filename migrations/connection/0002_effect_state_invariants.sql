-- HLD sections 19 and 20 require durable state machines. Enum checks alone
-- permit terminal-state regressions and bypassed dispatch admission.
...
BEGIN
  ...
  ELSIF TG_TABLE_NAME = 'connection_dispatches' THEN
    IF (OLD.status = 'PENDING' AND NEW.status IN ('SUBMISSION_STARTED', 'FAILED'))
      OR (OLD.status = 'SUBMISSION_STARTED' AND NEW.status IN ('SUCCEEDED', 'FAILED', 'UNCERTAIN'))
      OR (OLD.status = 'UNCERTAIN' AND NEW.status = 'SUCCEEDED') THEN
      RETURN NEW;
    END IF;
...
CREATE TRIGGER connection_calls_status_transition
BEFORE UPDATE OF status ON connection_calls
FOR EACH ROW EXECUTE FUNCTION connection_enforce_status_transition();

CREATE TRIGGER connection_effects_status_transition
BEFORE UPDATE OF status ON connection_effects
FOR EACH ROW EXECUTE FUNCTION connection_enforce_status_transition();

CREATE TRIGGER connection_dispatches_status_transition
BEFORE UPDATE OF status ON connection_dispatches
FOR EACH ROW EXECUTE FUNCTION connection_enforce_status_transition();

CREATE TRIGGER connection_reconciliation_jobs_status_transition
BEFORE UPDATE OF status ON connection_reconciliation_jobs
FOR EACH ROW EXECUTE FUNCTION connection_enforce_status_transition();
