...
3. `patches/0003-protected-connection-client.patch`：当前的 Linux aarch64 内存观察限制、
   ...
   原记录核实和有时限的 metadata-only 补核实，以及 canonical corpus 测试源码。
...
- 156 个共同 frame case、3 个 framing case、34 个 RFC 8785 vector 已写入 native 测试。
...
会在 dispatch 前拒绝。普通原 MCP hook 路径保持原有实现。当前没有跨进程 receipt/descriptor
恢复输入；重启后不能通过模型历史、nonce 查询、Provider 重发或 TS 代发 GET 伪造补核实。
...
它不改变现有 installer、Dockerfile 或 release pin；当前尚无此入口的成功 CI 构建证据。

[构建入口](build-linux-aarch64.sh) 先通过 `paths` 固定目录，再按
`prepare`、`musl`、`v8`、`build`、`seal` 分步运行；
...
Schema/corpus/coverage、工具版本、构建命令和最终文件 SHA。SBOM 使用仓库固定 Trivy
安装器，只描述 Cargo.lock 源码依赖，不证明静态链接 C/C++ 或最终 binary 依赖覆盖完整。
