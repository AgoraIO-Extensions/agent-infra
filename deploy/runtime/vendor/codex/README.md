...
[build-input 索引](build-input-v1.json.parts.json) 对应的原始 JSON 记录上游 commit、Cargo/Bazel lock、
...
8. `patches/0008-native-compaction-test-fixtures.patch`：保留模型切换压缩断言，
   使用隔离子进程验证 hook 缺失通道及明确拒绝，生产源码不变。

超过 64 KiB 的输入按源文件/hunk、JSON 条目或许可证段落边界优先拆为有序文本片段。
`<原路径>.parts.json` 记录原输入及每片的长度和 SHA-256；`vendor_inputs.py` 在消费时
逐片核对并重建原始字节，不重新序列化 JSON，也不规范化换行。片段不能单独作为 patch 应用。
未分片原路径仍可读取。源码应用及候选构建入口自动处理片段；固定 build-input、patch、
许可证和已有 candidate 的原始摘要保持不变，缺片、错序、损坏或路径逃逸均拒绝。
源码准备使用仓库要求的 Node.js 24；共享 callback corpus fixture 由其 loader 还原，
该 loader 缺失、Node.js 版本不符或 corpus 摘要不符时拒绝继续。

第三份补丁中完整新增 corpus 的 hunk 使用固定 `callback-corpus-patch-v1` 生成段，
复用上述语料并还原原 diff header 与逐行加号，不重复保存同一份大型语料。
生成段与完整补丁仍分别校验原始长度及 SHA-256；索引不能指定命令或自定义生成器。
...
第四个补丁的依赖许可证及逐项来源见
[dependency-updates-NOTICE 索引](licenses/dependency-updates-NOTICE.txt.parts.json)。
候选包中的许可证仍使用原文件名和完整原始字节，不包含分片索引或片段。
...
必须按序应用全部八份补丁，并核对重建的 `build-input-v1.json` 的逐文件 SHA、Cargo/Bazel
