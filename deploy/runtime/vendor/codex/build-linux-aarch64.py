...
LICENSE_FILES = (
    "dependency-updates-NOTICE.txt",
    "ryu-js-1.0.3-APACHE.txt",
    "ryu-js-1.0.3-BOOST.txt",
    "serde_json_canonicalizer-0.3.2-MIT.txt",
)
...
def input_digest(path):
    return "sha256:" + hashlib.sha256(read_input(path)).hexdigest()


def copy_licenses(vendor, destination):
    destination.mkdir()
    for name in LICENSE_FILES:
        target = destination / name
        target.write_bytes(read_input(vendor / "licenses" / name))
        target.chmod(0o644)
...
def verify_inputs(vendor, source):
    manifest_raw = read_input(vendor / "build-input-v1.json")
    ...
    hashes = {"build-input-v1.json": "sha256:" + hashlib.sha256(manifest_raw).hexdigest()}
    for entry in manifest["patches"]:
        actual = input_digest(relative_file(vendor, entry["path"]))
    ...
    for name, expected in manifest["callbackInputs"].items():
        actual = input_digest(relative_file(vendor, name))
...
class Builder:
    ...
    def build(self):
        inputs = self.inputs()
        cargo_jobs = os.environ.get("CARGO_BUILD_JOBS")
        require(cargo_jobs in {"2", "3"}
        ...
        copy_licenses(self.vendor, legal / "licenses")
        ...
        save(self.diag / "build.json", {
            ...
            "jobs": int(cargo_jobs), "incremental": False, "inputSha256": inputs, "tools": tools,
    ...
    def seal(self):
        ...
        record["builderSha256"] = {str(path.relative_to(self.repo)): digest(path) for path in (
            ...
            self.vendor / "apply-source.py", self.vendor / "vendor_inputs.py",
            self.repo / ".github/scripts/install-trivy.mjs",
        ...
        binary_paths = {"bundle/" + relative for relative in BINARIES.values()}
        expected_modes = {name: 0o755 if name in binary_paths else 0o644
                          for name in expected_files}
        def archive_mode(member):
            member.mode = expected_modes[member.name]
            return member
