...
// Call only for this invocation's staging or already-replaced backup. Never
// follow links from the previous installation into unrelated directories.
async function removeIsolatedTree(path) {
	if ((await lstat(path)).isDirectory()) {
		await chmod(path, 0o700);
		for (const name of await readdir(path))
			await removeIsolatedTree(join(path, name));
	}
...
if (release.schemaVersion !== undefined || release.distribution !== undefined) {
...
try {
	...
	try {
		...
		previousRoot = await mkdtemp(
			join(installParent, ".codex-install-previous-"),
		);
