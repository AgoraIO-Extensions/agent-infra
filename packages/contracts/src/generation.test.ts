...
describe("standard contract artifacts", () => {
	it("generates deterministic OpenAPI 3.1 and JSON Schema 2020-12", () => {
		...
		expect(artifacts.pilotDelegatedJsonSchema.$defs).toHaveProperty(
		...
		const validateSseMessage = ajv.compile({
			$ref: `${artifacts.pilotSseJsonSchema.$id}#/$defs/ConversationSseMessageV1`,
		});
		expect(
			validateSseMessage({
				schemaVersion: 1,
