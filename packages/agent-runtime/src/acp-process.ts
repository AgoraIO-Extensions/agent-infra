...
import type { AcpLaunch } from "./acp-session.js";
...
async function retire(file: DurableJsonFile<{ owner?: Owner }>) {
	...
	if (groupExists(owner.pid)) {
		...
		if (
			!new RegExp(
				`(?:^|\\s)AGENT_INFRA_ACP_OWNER=${owner.token}(?:\\s|$)`,
			).test(environment)
		)
			throw unavailable();
		try {
...
export async function retireAcpProcess(directory: string) {
	await retire(await ownerFile(directory));
}

/** One owned process group per native Session; persist ownership before sending any ACP request. */
export async function spawnAcpProcess(
	...
	const child = spawn(launch.command, launch.args, {
		cwd,
		env: { ...launch.env, AGENT_INFRA_ACP_OWNER: token },
