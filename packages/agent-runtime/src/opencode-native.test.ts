...
it.skipIf(!process.env.OPENCODE_EXECUTABLE)(
	"uses the selected Messages model and effort through unmodified OpenCode and resumes confirmed history",
	async () => {
		...
		let driver = await openOpenCodeRuntime(options);
		try {
			const command = {
			...
			await driver.close();
			driver = await openOpenCodeRuntime(options);
...
it.skipIf(!process.env.OPENCODE_EXECUTABLE).each(["read", "write", "edit"])(
	...
	async (toolName) => {
					...
					delta: {
						...
						partial_json: JSON.stringify({
							...
							...(toolName === "write"
								...
								: toolName === "edit"
									? { oldString: "prefix", newString: "changed" }
									: {}),
						}),
					},
				});
		...
		const address = server.address();
		if (!address || typeof address === "string") throw new Error();
		const driver = await openOpenCodeRuntime({
		...
		try {
			...
			expect(JSON.stringify(history)).not.toContain("SYNTHETIC_FOREIGN_CANARY");
