...
export async function startPostgresTestDatabase(
	...
	try {
		await execFile("docker", [
			...
			containerName,
			"--env",
			"POSTGRES_USER=connection_test",
			"--env",
			"POSTGRES_PASSWORD=connection_test_password",
			"--env",
		...
		const databaseUrl = `postgres://connection_test:connection_test_password@127.0.0.1:${port}/connection_test`;
