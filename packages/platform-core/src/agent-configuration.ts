...
function denseArray(value: unknown, maxLength: number): unknown[] {
	if (!Array.isArray(value) || value.length > maxLength) invalidCommand();
	for (let index = 0; index < value.length; index += 1) {
		if (!Object.hasOwn(value, index)) invalidCommand();
	}
	return [...value];
}
...
function parseActorContext(
	...
	const values = exactObject(actorContext, ["schemaVersion", "actorId"]);
	if (values.schemaVersion !== 1 || !isText(values.actorId, idMaxBytes)) {
		invalidCommand();
	}
	return { schemaVersion: 1, actorId: values.actorId };
}
...
function parseAdmittedModel(
	...
	if (
		...
		values.defaultOptionId !== requested.defaultOptionId ||
		values.defaultReasoningLevel !== requested.defaultReasoningLevel
	) {
		invalidCommand();
	}
	const inputs = denseArray(values.options, maxModelOptions);
	if (inputs.length !== requested.options.length) invalidCommand();
	...
	for (const inputOption of inputs) {
		...
		if (
			...
			!sameValue(expected.reasoningLevels, reasoningLevels.toSorted()) ||
	...
	return {
		async update(commandInput, actorContextInput) {
			...
			if (command.changes.source) {
				...
				>;
				try {
					admission = await dependencies.imageAdmission.admitImage({
						schemaVersion: 1,
						agentId: command.agentId,
						requestId: command.requestId,
						traceId: command.traceId,
				...
				let admittedSource: AgentConfigurationSourceV1;
				try {
					admittedSource =
						admission.status === "admitted"
							? parseAdmittedSource(admission.source)
							: current.source;
				} catch {
					throw new AgentConfigurationError("not_admitted");
				}
			...
			if (command.changes.modelConfiguration) {
				...
				>;
				try {
					admission = await dependencies.modelAdmission.admitModels({
						schemaVersion: 1,
						agentId: command.agentId,
						requestId: command.requestId,
						traceId: command.traceId,
			...
			if (command.changes.secrets) {
				...
				if (command.changes.secrets.length > 0) {
					...
					>;
					try {
						admission = await dependencies.secretAdmission.admitSecrets({
							schemaVersion: 1,
							agentId: command.agentId,
							requestId: command.requestId,
							traceId: command.traceId,
					...
					>();
					for (const metadataInput of admission.secrets) {
						let metadata: Record<string, unknown>;
						try {
							metadata = exactObject(metadataInput, [
								"name",
								"secretId",
								"version",
								"isSet",
			...
			if (command.changes.actions) {
				...
				>;
				try {
					admission = await dependencies.actionAdmission.admitActions({
						schemaVersion: 1,
						agentId: command.agentId,
						requestId: command.requestId,
						traceId: command.traceId,
				...
				let admittedActions: AgentConfigurationActionV1[];
				try {
					admittedActions =
						admission.status === "admitted"
							? canonicalActions(admission.actions)
							: [];
				} catch {
					throw new AgentConfigurationError("not_admitted");
				}
			...
			if (command.changes.channels) {
				...
				>;
				try {
					admission = await dependencies.channelAdmission.admitChannels({
						schemaVersion: 1,
						agentId: command.agentId,
						requestId: command.requestId,
						traceId: command.traceId,
				...
				let admittedChannels: AgentConfigurationRecordV1["channels"];
				try {
					admittedChannels =
						admission.status === "admitted"
							? canonicalChannelBindings(admission.channels)
							: [];
				} catch {
					throw new AgentConfigurationError("not_admitted");
				}
