...
function audit(
	...
	return {
		schemaVersion: 1,
		auditId: `audit-${rotation.rotationId}-${candidate.secretVersion}-${outcome}`,
...
function retirementAudit(command: RetireSecretKeyCommandV1) {
	return {
		schemaVersion: 1,
		auditId: `audit-retire-${command.keyVersion}`,
	...
	it("recovers before commit and atomically rotates without dropping the old active version", async () => {
		...
		await expect(
			store.commitReencryption({
				...
				auditEvent: audit(first.candidate),
			}),
		...
		await expect(
			store.retireKey({
				command: retirement,
				auditEvent: retirementAudit(retirement),
			}),
		...
		const secondCommit = {
			...
			auditEvent: audit(second.candidate),
		};
	...
	it("rolls back ciphertext, audit, and progress on a duplicate DEK fingerprint", async () => {
		...
		await expect(
			store.commitReencryption({
				...
				auditEvent: audit(next.candidate, collisionCommand),
			}),
	...
	it("keeps a key-missing rotation retryable but terminates authenticated corruption", async () => {
		...
		await expect(
			store.recordRejection({
				...
				auditEvent: audit(first.candidate, rejectionCommand, "failed"),
			}),
		).resolves.toBe(true);
		...
		await expect(
			store.recordRejection({
				...
				auditEvent: audit(retried.candidate, rejectionCommand, "rejected"),
			}),
		).resolves.toBe(true);
