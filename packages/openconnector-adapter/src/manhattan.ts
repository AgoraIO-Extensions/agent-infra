...
const providerReleaseId = "manhattan-connection-v1";
...
export const manhattanConnectionCatalog = {
	actions: [
		{
			...
			id: "manhattan.get_current_user@v1",
			inputSchema: {
		...
		},
		{
			...
			effect: "READ" as const,
			id: "manhattan.list_sdk_dumps@v1",
			inputSchema: {
				additionalProperties: false,
				properties: {
	...
	authProfile: { gatewayHeader: "apiKey", personalCredential: "bearer-token" },
	deploymentProfile: {
		apiOrigin,
	...
	implements ProviderCredentialConnector, ProviderExecutor
{
	...
	async execute(input: {
		...
		if (!token) throw invalidCredential("Manhattan OAuth token is required");
		if (input.action === "manhattan.get_current_user")
			return this.request("/api/connection/whoami", token);
		if (input.action === "manhattan.list_sdk_dumps")
			return this.request("/api/connection/sdk/dumps", token, input.input);
