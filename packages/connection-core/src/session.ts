...
export class BrowserSessionService {
	...
	async resolve(
		...
		if (!entry.exists) {
			await this.principals.disable(record.principalId);
			await this.sessions.revoke(record.id, this.now());
			return undefined;
		}
		const current = await this.principals.findById(record.principalId);
		return current?.status === "active" &&
			current.issuer === record.issuer &&
			current.uid === record.uid &&
			current.recoveryGeneration === record.recoveryGeneration
			? current
			: undefined;
	}
