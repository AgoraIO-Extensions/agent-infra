...
Connection 相关工作还必须遵守 [Connection M1 HLD](docs/architecture/HLD-connection-M1.md)。实现、测试、部署配置和文档都要先映射到适用的 HLD 条款；发现与 HLD、PRD 或工程 Spec 的偏差、缺失门禁或相互冲突时，先报告并取得 HLD/ADR 的明确更新或评审批准。不得以开发 fixture、环境变量、部署说明或兼容性开关绕过 HLD 门禁。
