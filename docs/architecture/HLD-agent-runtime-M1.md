...
| 准入投递 | 原子占用 Conversation 与 Agent 容量，重验授权/能力，按 8.1 取得租约并持久化投递状态 | 按原 executionId/operationId、输入/选择和 fence 持久接受、拒绝或报告 unknown |
...
- 首次受理在同一用例事务内检查 Agent 状态与等待容量，分配顺序和平台规定的等待期限，并保存表中受理记录。启动/更新中允许等待；停止、停用或故障未恢复时拒绝，任务提交不能隐式启动或恢复 Agent。容量满时事务不创建新任务；受理响应只能在业务记录、工作项与必要审计均已提交后返回。
...
- 关联只接受绑定原 Execution 的可信工具采集及 Connection 真实调用记录。摘要中的自报 callId 或任意相同字符串不能形成绑定；缺失/未知如实展示并沿原调用核实。两侧分别在受控 API/页面查询，关联不授予权限；Connection 内部 OAuth/LDAP/Grant 协议由其 HLD 维护。
...
- 取消故障注入覆盖等待与认领并发、投递前后崩溃、旧 Worker 迟到提交、unknown、停止未确认与自然完成竞态、代次隔离和等待任务收敛；必须证明旧 Turn 无剩余副作用才启动下一任务，不能用超时/换 executionId/新 Session 掩盖不确定结果。
