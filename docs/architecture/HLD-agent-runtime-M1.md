...
- 普通消息、补充指令和重新生成请求必须携带非空 `Idempotency-Key`，值只允许 `1..128` 个 ASCII 字母、数字、`.`、`_`、`~` 或 `-`，并作为区分大小写的不透明字符串处理。浏览器为一次逻辑提交生成 Key 并在传输重试时复用；Channel 层从可信渠道消息 ID 派生符合该格式的稳定 Key。`platform-api` 在任何写入前拒绝缺失或格式无效的 Key，并根据已确定的事务分支生成 `message`、`supplementary-instruction` 或 `regenerate` 的服务端 `commandType`，以非空字段建立 `(conversationId, actorId, commandType, Idempotency-Key)` 唯一约束；幂等重放必须使用相同的完整元组查询。`actorId` 和 `commandType` 均由服务端生成，不能接受调用方提交或覆盖。
- 同一 `actorId` 和 `commandType` 下，同一 Key 和相同消息、附件、模型选择再次提交时，普通消息返回原 Message 与初始 Execution，补充指令返回原 Message 与原绑定的 Execution；同一 Key 对应不同内容时返回冲突。不同 `actorId` 或 `commandType` 的 Key 独立生效。
