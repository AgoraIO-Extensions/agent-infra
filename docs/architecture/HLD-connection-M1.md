...
v3 的 password grant 账号体系与员工 IAM 分离，不能用于公司账号。v4 由员工在 Manhattan 已有 HCI 登录态下
自助创建固定 `manhattan.sdk.read` 的个人 PAT，Connection 只接受并加密保存 PAT，不接收公司密码；
Manhattan 按 PAT 持有人与实时 RBAC 授权执行 Action。机器 key 只由 Secret Manager 注入，不得
