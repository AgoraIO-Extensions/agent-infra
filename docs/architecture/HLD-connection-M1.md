...
flowchart LR
    User["公司员工或管理员"]
    Web["Web SPA"]
    Channel["企微或自定义 WebUI 入口"]
    Platform["platform-api / Tool Gateway"]
    ...
    Recovery["Recovery Control / Evidence Journal"]
    Egress["受控 Egress Proxy"]
...
flowchart TB
    subgraph Edge["公司入口"]
        Gateway["Identity-aware Gateway / Ingress"]
    end

    subgraph PlatformNS["Platform trust zone"]
        PA["platform-api replicas"]
        PW["platform-worker replicas"]
        PR["platform recovery-command listener\n独立端口/router/Service"]
        PDB[("Platform PostgreSQL HA\nwriter endpoint / platform role")]
        AgentPod["Agent Pods"]
    end

    subgraph ConnectionNS["Connection trust zone"]
        CA["connection-api replicas\nbusiness listener + PostgreSQL lease workers"]
        MR["management-read listener\n独立端口/router/Service"]
        CR["recovery-command listener\n独立端口/router/Service"]
        CDB[("Connection PostgreSQL HA\nwriter endpoint / connection role")]
    end

    subgraph SecurityServices["公司安全服务"]
        IDP["Identity / Organization"]
        KMS2["KMS / Secret Service"]
        OBJ["Private object storage\nconditional capability"]
        DR["SRE Recovery Control\noutside both DB PITR domains"]
        Journal["Signed Recovery Evidence Journal\nappend-only / outside PITR domains"]
        Diagnostic["SRE diagnostic identity"]
        Runner["short-lived recovery runners"]
    end

    subgraph Egress["受控出口"]
        EgressPolicy["Provider Egress Proxy\nMAC + DNS + take-once admission"]
...
```mermaid
flowchart LR
    API["apps/connection-api\nHono adapters"] --> Core["packages/connection-core\n领域和用例"]
    Core --> Boundaries["connection-core boundary contracts"]
    Store["packages/connection-store\nDrizzle implementation"] --> Boundaries
    OC["OpenConnector pinned implementation"] --> Boundaries
    IdentityA["Identity production client"] --> Boundaries
    KMSA["KMS production client"] --> Boundaries
    PlatformA["Platform internal client"] --> Boundaries
    Observe["Observability library"] --> Core
```
...
```mermaid
erDiagram
    PROVIDER ||--o{ PROVIDER_RELEASE : has
    PROVIDER ||--o{ ACTION : defines
    ACTION ||--o{ ACTION_VERSION : versions
    CONNECTION_INTENT }o--|| PROVIDER : targets
    CONNECTION_INTENT ||--o{ CONNECTION_INTENT_SHARED_SCOPE : freezes_initial_scope
    CONNECTION_INTENT ||--o{ OAUTH_TRANSACTION : authorizes_by
    OAUTH_TRANSACTION ||--o{ OAUTH_EXCHANGE_ATTEMPT : exchanges_by
    OAUTH_EXCHANGE_ATTEMPT ||--o{ OAUTH_ACCOUNT_PROFILE_ATTEMPT : resolves_account_by
    CONNECTION_INTENT ||--o{ API_CREDENTIAL_VALIDATION_ATTEMPT : validates_by
    PROVIDER ||--o{ CONNECTION_ACCOUNT : authenticates
    CONNECTION_ACCOUNT ||--o{ CONNECTION_SHARED_SCOPE : grants_discovery
    CONNECTION_ACCOUNT ||--o{ CONNECTION_USER_ENTITLEMENT : grants_continuity
    CONNECTION_USER_ENTITLEMENT ||--o{ CONNECTION_USER_ENTITLEMENT_EVIDENCE : versions_evidence
    CONNECTION_ACCOUNT ||--o{ CREDENTIAL_VERSION : protects
    CONNECTION_ACCOUNT ||--o{ ACTION_CALL : executes_with
    ACTION_VERSION ||--o{ ACTION_CALL : invokes
    ACTION_CALL ||--o{ ACTION_ATTEMPT : retries_as
    ACTION_CALL ||--o{ ACTION_CALL_PAYLOAD : protects
    ACTION_CALL ||--o{ LOGICAL_EFFECT : owns
    ACTION_ATTEMPT ||--o{ EFFECT_DISPATCH : performs
    LOGICAL_EFFECT ||--o{ EFFECT_DISPATCH : dispatched_as
    EFFECT_DISPATCH ||--o{ PROVIDER_EGRESS_HOP : sends_as
    OAUTH_EXCHANGE_ATTEMPT ||--o{ PROVIDER_EGRESS_HOP : sends_as
    OAUTH_ACCOUNT_PROFILE_ATTEMPT ||--o{ PROVIDER_EGRESS_HOP : queries_as
    PROVIDER_EGRESS_HOP ||--o| EGRESS_ASSERTION_ADMISSION : admitted_once
    LOGICAL_EFFECT ||--o{ ACTION_EFFECT_RESOLUTION_PROPOSAL : proposed_for
    ACTION_EFFECT_RESOLUTION_PROPOSAL ||--o| ACTION_EFFECT_RESOLUTION : approved_as
    ACTION_CALL ||--o{ CONNECTION_AUDIT_EVENT : records
    OAUTH_TRANSACTION }o--|| PROVIDER : starts_for
    OAUTH_TRANSACTION }o--o| CONNECTION_ACCOUNT : reconnects
```
...
stateDiagram-v2
    ...
    VALIDATING --> PUBLISHED: 管理员发布
    PUBLISHED --> DEPRECATED: 有替代版本
    PUBLISHED --> DISABLED: 安全或业务停用
    DEPRECATED --> DISABLED: 到期或强制停用
    DISABLED --> [*]
```
...
```mermaid
stateDiagram-v2
    [*] --> ACTIVE: 鉴权、账号识别和Credential提交成功
    ACTIVE --> DEGRADED: Provider 暂时故障
    DEGRADED --> ACTIVE: 健康恢复
    ACTIVE --> REAUTH_REQUIRED: Credential 永久失效
    DEGRADED --> REAUTH_REQUIRED: invalid_grant
    ACTIVE --> DISCONNECTED: Personal owner或Shared管理员断开
    DEGRADED --> DISCONNECTED: Personal owner或Shared管理员断开
    REAUTH_REQUIRED --> DISCONNECTED: Personal owner或Shared管理员明确断开
    REAUTH_REQUIRED --> ACTIVE: reconnect transaction 同账号成功
    DISCONNECTED --> ACTIVE: reconnect transaction 同账号成功
    ACTIVE --> DISABLED: 管理员禁用共享连接
    DEGRADED --> DISABLED: 管理员禁用共享连接
    REAUTH_REQUIRED --> DISABLED: 管理员禁用共享连接
    DISCONNECTED --> DISABLED: 管理员禁用共享连接
```
...
```mermaid
flowchart TD
    Start["回调取得 VerifiedExternalAccount"] --> Target{"是否针对已有 connectionId"}
    Target -- 否 --> New["创建新 Connection ID"]
    Target -- 是 --> Compare{"fingerprint 是否相同"}
    Compare -- 是 --> Rotate["原子切换 Credential\n保持 Connection ID\n提升 executionFence"]
    Rotate --> Resume["原 Grant 从暂停恢复\n仍受当前 Action/scope 交集约束"]
    Compare -- 否 --> Separate["不得覆盖旧 Connection"]
    Separate --> New2["创建新的 Connection ID\n旧 Grant 不迁移"]
    New2 --> Prompt["用户在 Platform 明确切换并重新授权"]
```
...
stateDiagram-v2
    [*] --> PENDING
    PENDING --> EXCHANGING: callback 原子消费 state
    PENDING --> FAILED_DEFINITE: 授权拒绝或前置门禁失败
    PENDING --> EXPIRED: TTL 到期
    EXCHANGING --> COMPLETED: token candidate + durable profile evidence + Credential 提交
    EXCHANGING --> FAILED_DEFINITE: 明确拒绝或校验失败
    EXCHANGING --> UNCERTAIN: token endpoint 结果未知且不可安全重复
    COMPLETED --> [*]
    FAILED_DEFINITE --> [*]
    EXPIRED --> [*]
    UNCERTAIN --> [*]
...
| `confirmedActionSnapshotId` | 不可变的 `actionId + authorizationDigest` 授权集合；另含展示 exact version 证据 |
...
Preview 后 Owner exact version 若变化，只有当前 Owner 集的 `(actionId, authorizationDigest)` 多重集合与 preview 完全相等时才可提交；此时审计同时记录 displayed/current exact version。任何 digest、Action 增删或语义变化都 stale，不能借“兼容升级”确认用户未见过的权限。
...
```mermaid
stateDiagram-v2
    [*] --> ACTIVE: 用户确认 Agent + Connection + Action 快照
    ACTIVE --> PAUSED_CONNECTION: 断开或 Credential 失效
    ACTIVE --> PAUSED_RECOVERY: Platform recovery generation 变化
    PAUSED_CONNECTION --> ACTIVE: 同账号重连且 live 条件恢复
    PAUSED_CONNECTION --> PAUSED_RECOVERY: Platform recovery generation 变化
    PAUSED_RECOVERY --> REPLACED: 旧版本；重确认创建新 ACTIVE Grant
    PAUSED_RECOVERY --> REVOKED: 用户撤销
    PAUSED_RECOVERY --> REPLACED: 用户重新确认并明确换号
    PAUSED_RECOVERY --> TERMINATED: 权威主体或范围失效
    ACTIVE --> REPLACED: 扩权或换号创建新 ACTIVE Grant
    PAUSED_CONNECTION --> REPLACED: 用户明确切换 Connection
    ACTIVE --> REVOKED: 用户撤销
    PAUSED_CONNECTION --> REVOKED: 用户撤销
    ACTIVE --> TERMINATED: 用户禁用或共享范围失效
    PAUSED_CONNECTION --> TERMINATED: 不可恢复权威条件失效
    REPLACED --> [*]
    REVOKED --> [*]
    TERMINATED --> [*]
```
...
2. 验证 identityAssertion freshness/monotonic epoch、当前 Platform recovery generation和policy validation generation、AgentAuthorizationRoot已在本代复核、Grant的本代重确认与entitlement binding，并重新检查 Agent 范围、渠道、Owner exact selection、`actionId + authorizationDigest` 用户确认和 `authorizationFence`。Mutating Action还要在事务提交点验证decision lease未过期，external current与Platform mirror的control version、mutation generation/state、evidence kind/ref/checksum/issuedAt/expiresAt、风险窗口、Catalog mutating-set version/checksum和future topology ref/checksum/expiry完全一致；发现 definitive entitlement loss 时按17.5原子终结Grant后拒绝。
...
sequenceDiagram
    ...
    participant U as User revoke request

    par Permit redeem
        C->>P: redeem(token, callId, actionVersionId, argsHash, preauthorizationRequestHash)
        P->>D: lock AgentAuthorizationRoot + GrantSlot + current Grant
    and Revoke
        U->>P: revoke Grant
        P->>D: lock same AgentAuthorizationRoot + GrantSlot
    end
    alt redeem transaction commits first
        D-->>C: REDEEMED
        Note over C: Platform 授权已接受，调用进入 in-flight
        D-->>U: Grant REVOKED for future calls
    else revoke transaction commits first
        D-->>U: Grant REVOKED
        D-->>C: redeem denied
        Note over C: 确认没有 Provider 出站
    end
```
...
Platform 不通过重复创建新 ActionCall 来恢复丢失响应。相同 `toolCallId`/幂等键必须解析到原 `callId`，直到得到持久化终态或 `UNCERTAIN`。
...
同一 `(callId, logicalEffectKey)` 再次登记时，request hash、endpoint policy、method 或语义版本任一不同都禁止出站，并把已持久化ActionCall收敛为`FAILED_DEFINITE`及资源condition `ACTION_LOGICAL_EFFECT_CONFLICT`；executor 不能复用 effect key 把第二个业务效果伪装成 retry。

### 20.2 ActionCall 状态机

```mermaid
stateDiagram-v2
    [*] --> PREPARED: 幂等 claim + 持久化请求
    PREPARED --> DENIED: Permit 无效或授权撤销
    PREPARED --> EXPIRED_BEFORE_AUTH: Invocation 总 deadline 到期
    PREPARED --> CANCELLED_NO_MUTATION: 明确取消
    PREPARED --> AUTHORIZED: Permit 已兑换
    AUTHORIZED --> CANCELLED_NO_MUTATION: 尚无 Effect intent 时取消
    AUTHORIZED --> DENIED_LOCAL: live Connection/Action/scope 检查失败
    AUTHORIZED --> DISPATCH_READY: Attempt + Effect + Dispatch PREPARED
    DISPATCH_READY --> CANCELLED_NO_MUTATION: 零mutating submission且取消成功
    DISPATCH_READY --> DENIED_LOCAL: fence 变化且尚未出站
    DISPATCH_READY --> DISPATCHING: Dispatch SUBMISSION_STARTED
    DISPATCHING --> CANCELLED_NO_MUTATION: 全部mutating Dispatch未开始且read已收敛
    DISPATCHING --> DENIED_LOCAL: 先前read已收敛，后续mutating Dispatch在本地门禁前被拒绝
    DISPATCHING --> DISPATCHING: 准备下一个逻辑 Effect，或允许安全重试且 budget 未耗尽
    DISPATCHING --> SUCCEEDED: 结果和审计已持久化
    DISPATCHING --> FAILED_DEFINITE: 可证明无成功效果或 Provider 明确失败
    DISPATCHING --> UNCERTAIN: 可能已产生效果但无法证明结果
    UNCERTAIN --> RECONCILING: 只读状态查询；人工解析仅在 G-04 批准后
    RECONCILING --> SUCCEEDED: 确认已成功
    RECONCILING --> FAILED_DEFINITE: 确认未生效或失败
    RECONCILING --> UNCERTAIN: 仍无法判断
```
...
| `RETRY_PENDING` | 已证明本次 Dispatch 未生效且 policy/budget 允许安全 retry；下一次发送仍引用同一 LogicalEffect |
...
一旦 mutating Effect `CONFIRMED`，Call 不得因输出 Schema、payload 持久化或回调投递错误改写为 `FAILED_DEFINITE`。这些错误保持外部效果成功事实，结果字段标记不可用并告警；audit/outbox 由本地事务恢复。
...
每次受控重试都创建新的 ActionAttempt 和新的 EffectDispatch，并各自使用新 ID；它们必须继续引用同一个 LogicalEffect。Provider idempotency key 从 `callId + logicalEffectKey + provider policy version` 确定性派生，对同一 LogicalEffect 跨 attempt 保持稳定。重试次数、backoff 和 timeout 是 Provider/ActionVersion policy，不由 Agent 参数决定。
...
Preview item 保存 exact `action_version_id/authorization_digest/required_scopes/effect_class`。Intent 只用于证明用户确认页面内容，不直接授权 Provider 调用。
...
- `current_permit_sequence`、可选 `bound_call_id`、correlation IDs。Invocation 声明 unique `(id, execution_id, tool_call_id)`；`(id, current_permit_sequence) -> execution_permit(invocation_id, permit_sequence)` 使用 DEFERRABLE composite FK，禁止 current pointer 指到另一 Invocation 的 Permit。
...
只有当前 Permit 已确认 EXPIRED 且未兑换、两侧 recovery generation 和冻结 target 未变时，Invocation 才能在重新授权检查后签发下一 sequence。一旦任何 Permit REDEEMED，Invocation 永久绑定同一 callId，不得续签为另一次调用；恢复出的已绑定 Invocation 只允许关联查询，不创建新 Permit。
...
| execution | executor build digest/export、timeout、result size、retry/reconcile policy、egress profile |
...
| `NOT_APPLICABLE` | 必须为空 | 必须为空 | AUTH/SCOPE/DEFINITE/RESULT_INVALID等不可重试类别；使用原exact terminal reason，禁止successor |
...
- unique `(caller_service,idempotency_key)`和unique `(caller_service,requested_invocation_id)`共同保证同一Invocation即使错误使用第二个key也不能prewrite第二个Call；PREPARED claim的冲突键是NOT NULL、write-once的`preauthorization_hash_version + preauthorization_request_hash`，redeem后还必须匹配完整`logical_request_hash`。
- unique non-null `invocation_id`、`redeemed_permit_id` 和 `(execution_id, tool_call_id)`；短期 Permit generation 不另建 Call。
...
`PREPARED/RUNNING` Attempt可以持lease、登记新Effect或创建Dispatch。`RETRY_PENDING`必须同时满足
`retry_decision=SUCCESSOR_CREATED`、存在唯一下一sequence `PREPARED` successor且Call pointer已移动；
...
| `request_hash`、`effect_policy_version`、`endpoint_policy_id`、`method`、`path_template` | 固定逻辑效果语义；不保存含 query Secret 的完整 URL |
| `provider_idempotency_key_ciphertext/hash` | 对同一逻辑 Effect 跨 attempt 固定，只在 Provider 需要时受控保存 |
...
只有 LogicalEffect policy允许retry且20.6 successor事务完整提交时才创建下一Dispatch；新Dispatch必须引用同一effectId和稳定Provider idempotency key，不能通过创建新Effect绕过去重。`FAILED_BEFORE_SUBMIT`只终结当前Dispatch；若允许retry，LogicalEffect进入`RETRY_PENDING`，否则在有durable未生效证据时进入`NOT_APPLIED`。没有proof时不得以budget耗尽把未知结果改写成NOT_APPLIED。
...
权威，唯一权威仍是PITR域外Journal Authority；本地行被PITR丢失后必须通过原scope重新`claimOperation`和
`resolveOperation`重建，不能分配新operation或根据本地空表判断“没有barrier”。
...
内部revision；actor-scoped GET从Authority claim绑定恢复目标，Browser不能用响应字段替换resource。首次202、相同key/hash重放和GET
必须逐字节使用同一schema；terminal 200继续使用各operation已登记的结果表示，不能把本类型扩成泛化成功envelope。
...
| `NOT_APPLICABLE` | OAuth callback与全部GET | 只读或redirect；不得写command claim。callback的一次性由state transaction保证 |

两个`ONLY_IF_G04_APPROVED` operation与两个`ONLY_IF_G17_APPROVED` terminal recovery operation在各自Gate未关闭时不进入active union。G-04关闭后两项以`HEADER_KEY + NO_PUBLIC_IN_PROGRESS`加入，每项同时生成22.4 strict request/result/status及25.1.7的7条normal edge；G-17关闭后restore以`HEADER_KEY`、resolve以`PROTOCOL_IDENTITY`加入，使用各自strict alternate conflict contract。
...
| `VIEW_CALL` | Platform或Connection Call UI；当前user或原execution association仍匹配 | `CALL_DETAIL` -> `getUserActionCall`或受保护internal GET | `CALL_VIEW_2M` | 回到本人调用列表；不显示目标是否仍存在 |
...
只绑定一个GET/read-like POST operation；`CALL_VIEW_2M`最大2分钟且绑定callId与actor/execution association；`NONE`要求
...
| `GET /api/v1/calls/{callId}` | 登录员工 | 本人脱敏详情、Effect 状态和下一步 | 无权与不存在统一响应；授权后复用`ActionCallRepresentationV1` |
...
`ConnectionIntentRepresentationV1`由Intent命令terminal replay、`GET /api/v1/connection-intents/{id}`和22.6 completion check复用，使用`oneOf + const + additionalProperties:false`严格union。联合判别字段为`status/pendingStage/condition.code?`，common字段只有`intentId/authMethod/expiresAt`；任一route不得复制为近似schema：
...
| `GET /api/v1/admin/safety-mutations/{operationId}` | 查询Catalog或Connection-safety安全命令 | 同发起admin/operation scope；返回barrier/materialization safe状态；无取消或删除route |
...
同key/hash重试先以原scope重新claim并核对同operation/claim checksum，再resolve/补落；safety-mutation GET由服务端按
同actor/route/resource从Authority读取claim envelope后执行相同resolve。相同key异hash为409，不能换operationId绕过未知结果。
...
**[本项目设计决策]** **[评审门禁 G-04]** `ONLY_IF_G04_APPROVED`候选契约中，Resolution proposal request只接受`expectedEffectStateRevision/proposedConclusion/evidenceKind/safeEvidenceRefs/providerObservedAt?/caseId/reasonCode`及`Idempotency-Key`，actor、current system admin状态、live `EFFECT_RECONCILE_PROPOSE` capability和step-up evidence由服务端会话解析；服务端固定`expiresAt=submittedAt+30 minutes`，调用方不能延长。Approve request不接受JSON业务字段，仅使用path proposalId、strong `If-Match` proposal checksum和`Idempotency-Key`；提交时重新解析checker的current system admin状态、live `EFFECT_RECONCILE_APPROVE` capability和独立step-up。同人批准、任一admin/capability撤销、复用同一审批会话、proposal过期/checksum不符、Effect revision stale、证据不足或跨Effect IDOR统一映射存在性安全的`404 RESOURCE_NOT_FOUND_OR_NOT_ALLOWED`且不写resolution；同key/hash响应丢失回放exact status/content-type/body/ETag，不执行第二次transition。G-04未批准时本段及下述类型不生成route/schema/client。
...
HTTP为`202`，同key/hash重试先重新claim原scope并校验返回同一operation/claim checksum，再resolve/补落同一record；
GET由服务端按同actor/route/resource读取Authority claim envelope后执行同一resolve，不能只查可被PITR回退的本地表。
...
不使用`HttpErrorEnvelopeV1`或`ConditionV1`，因此不改变52/26/2 registry。Restore相同Idempotency-Key/request逐字节回放同一结果，异hash只走上述409；resolve同protocol tuple逐字节回放同一witness。
...
ActionCall 用户查询必须同时过滤 `user_id=current_user`。Agent Owner 本身不增加查询普通用户 Call 的谓词；管理员查询走独立 route、RBAC 和审计。
...
| `ActionCallInternalReasonCodeV1` | `LOGICAL_EFFECT_BINDING_CONFLICT` | `FAILED_DEFINITE` | `CONCURRENCY` | `SELF_CONDITION:ACTION_LOGICAL_EFFECT_CONFLICT` |
...
| `approveEffectResolutionProposal` | `POST` | `/api/v1/admin/action-effect-resolution-proposals/{proposalId}:approve` | `409` | `IDEMPOTENCY_CONFLICT` |
...
`HttpErrorCodeV1`在本版恰有52个literal；上述四张normal edge表展开后必须覆盖52/52，允许一个code有多个exact emitter，但不允许任何code零emitter。G-04关闭时两个condition operation各展开7条edge，共增加14条`FailureEmitterEdgeV1`但不增加literal；未关闭时这些edge与route/schema/carrier一起为零active artifact。未列出的operation不能发出`HttpErrorCodeV1`；若后续route需要复用某code，必须新增一条完整edge并通过breaking/consumer评审，不能依赖middleware tag或相同path前缀自动继承。
...
4. 52个`HttpErrorCodeV1`、26个`ResourceConditionCodeV1`和2个`EligibilitySafeReasonCodeV1`都恰有已登记edge，三个code集合两两互斥；内部reason、ProviderFailure、禁止alias和unknown future token的edge数必须为零。
5. Browser/Internal/Tool Gateway三处`ActionCallRepresentationV1`用同一canonical serializer做逐字节golden test；ConnectionIntent command/GET/completion/revalidation同理。`CommandInProgressV1.pollAfterSeconds`固定在顶层，condition内不得复制。Eligibility golden另拒绝SAFE_DENY携loss、DEFINITIVE_LOSS缺loss、safe reason与disposition对调、非`CURRENT_PATH_LOST` root reason及任意三namespace跨carrier。
...
- `RETRY_ALLOWED`要求endpoint可安全重复且failure evidence为`PROVED_NOT_APPLIED`。自动worker链还要求deadline/policy/max允许，并在21.4同一事务取得`ACQUIRED` decision、创建唯一successor；OAuthTransaction是唯一user-triggered例外，failure观察先持久化`RETRY_SAME_INTENT + NONE/AWAITING_USER_ACTION`，用户命令事务才重验deadline/policy/max并取得budget、创建successor。
...
这3/12不计入25.1.7的67个logical operation definition、69个physical business/protocol instance、52个HttpError、26个
...
3. Authority在commit时填充sequence/previous hash/occurredAt/kid/signature并生成record checksum，因此调用方在append
   ...
   CLAIMED_NO_RECORD且无in-flight append时才可按第2步重试；APPENDED只补落返回的同一record。
4. 进程重启或本地claim投影丢失时先以原scope重新claim找回exact operation/claim checksum，再resolve；不能换operation。
   ...
   signed claim envelope；服务端随后用其中claim checksum/request hash调用resolve。该lookup不能append、改变head或
   省略actor/resource存在性检查。
...
| ID | 层级 | 必测内容 | 完成证据 |
| --- | --- | --- | --- |
| T-UNIT | Domain unit | 状态机、授权交集、revision/fence、canonical hash、terminal source/proof/core/body/signature/envelope/event/snapshot/ACK无环hash、错误映射、reconnect fingerprint、aggregate-specific reason/parent/public projection、Attempt/Dispatch/Reconcile predecessor+current pointer+冻结policy/max、Call首次submission单调门禁、ProviderFailure生成矩阵 | 表驱动覆盖每个合法/非法transition和拒绝原因；proof各层preimage排除自身和后续checksum且domain不可互代；首次submission后Call永不回退`DISPATCH_READY`，多read-effect后取消只在零mutating submission时进入`CANCELLED_NO_MUTATION`；read-before-write只允许在已有read收敛、mutating submission严格为零且后续live gate确定拒绝时`DISPATCHING -> DENIED_LOCAL`；max=`1/N`的`N-1/N/N+1`边界、retry allowed/exhausted/forbidden各有正反例；每个reason恰有一个state class、exact parent transition和public projection；每个ProviderFailure笛卡尔组合恰为一个disposition或`ILLEGAL` |
...
| T-ISO | Authorization isolation E2E | 用户/Agent/Connection/Shared/channel/账号禁用/组织移除；G-18 page-action current-user/target-Agent actor predicate、predecessor/acquiring Agent-Slot分离和旧Agent名称可见性 | Alice/Bob x Agent A/B负向矩阵；两组Agent/Slot逐字段置换和Browser字段注入全部拒绝；失去旧Agent访问权后只显示固定通用文案且response、timing、audit/telemetry均不泄露名称或存在性 |
| T-EFFECT | Action execution | read/write、多read-effect及read-before-write、四值retry disposition、timeout、ActionAttempt/Effect/Dispatch唯一successor、REQUEST/RESULT payload ownership、UNCERTAIN、独立reconcile attempt、exact resultViolation与ProviderFailure全矩阵；`ONLY_IF_G04_APPROVED`时再包含immutable proposal/second approval | 每个transport/crash点无重复副作用；已有read-only response但零mutating submission时取消收敛为`CANCELLED_NO_MUTATION`，mutating request可能开始后只能`CANCEL_REQUESTED`并保留真实结果；ACQUIRED successor与EXHAUSTED/FORBIDDEN终结均全原子且reason不同；read-only与idempotent-write的`OUTCOME_UNKNOWN + RETRY_EXHAUSTED\|RETRY_FORBIDDEN`分别按25.2唯一分流；ActionAttempt/Dispatch共享decision；payload不跨Call/type，reconcile不重放mutation。G-04 suite逐个kill proposal claim/insert/201及approval claim/revalidation/resolution/Effect-Call CAS/audit/outbox/200边界，same-key/hash始终回放exact status/content-type/body/ETag、异hash 409且无第二transition；同人、错`If-Match`、capability撤销、step-up复用、过期/stale均统一404和零resolution。G-04未批准时proposal/resolution schema、route、carrier、edge和fixture均不存在 |
...
25.1.7另做独立flat-edge golden：52/52 HttpError、26/26 ResourceCondition与2/2 EligibilitySafeReason均至少一个且只出现在各自法定carrier edge；三个literal namespace两两交集为空。每个edge引用存在且metadata逐字节一致的logical OpenAPI operation，每个active physical instance分别证明response/allowlist/serializer一致；OpenAPI response、physical route allowlist、serializer registration与flat snapshot双向集合相等；resource持久化后同一失败不得继续走pre-resource HttpError edge。跨carrier golden固定`SHARED_SCOPE_REVOKED`在eligibility edge可达数为0，`ELIGIBILITY_NOT_AVAILABLE`和`ELIGIBILITY_CURRENT_PATH_LOST`在HttpError/ResourceCondition edge可达数为0，并逐个拒绝safeReason、`SAFE_DENY|DEFINITIVE_LOSS`和lossEvidence required/forbidden集合的错配。固定断言67 logical=`63 ACTIVE+2 G04+2 G17`、69 physical=`65 ACTIVE+2 G04+2 G17`及3/12 operational；默认build为63/65，仅G04与仅G17分别为不同集合的65/67，两者都关闭为67/69。幂等carrier同时冻结默认`29+2 / 3-7-6-15`、仅G04 `31+2 / 3-7-6-17`、仅G17 `30+2 / 3-7-6-16`、两Gate `32+2 / 3-7-6-18`，后一tuple按四种policy顺序；比较operation ID集合而非只比基数。G-04关闭时两个normal operation各有request/CSRF/dependency/domain合计7条edge，未关闭时route/schema/carrier/edge均为零；52/26/2 literal不变。另固定断言25.1.7中9个internal operation恰有18条Identity/DB dependency edges，每个operation各有一条Identity和一条DB edge；显式置换operationId/method/path参数名/sorted targets/activation/deploymentInstance/owningDeployment/listener/port/audience/callerIdentityBinding/certificatePurpose/status/variant/schema/field path并逐个失败。ActionCall四入口和Intent各入口terminal bytes保持一致；Cancel覆盖same-key replay、different-key fresh observation和异hash冲突；`ActionResultInternalReasonCodeV1`在所有public schema中的可达edge数固定为零。
...
| Confirmation commit vs Owner Action change | 相同 `actionId + authorizationDigest` 可提交并记录 displayed/current exact version；否则 CONFIRMATION_STALE |
...
| 同 idempotency key 并发 | 一个 ActionCall/LogicalEffect，其他返回相同状态 |
...
| `ONLY_IF_G04_APPROVED`：Resolution proposal/approval并发 | proposal immutable；maker不能批准；两checker只有一个resolution/Effect CAS成功；任一current system admin或live IAM capability撤销、step-up session复用、expiry/revision变化均零transition，response loss回放同resolution；当前active fixture无该route/schema |
...
2. Permit issue commit后、token发送前；发送后响应丢失；redeem的commitment比较、Permit/Invocation/call+hash原子association、Platform commit后但Connection未知；introspect不得消费未兑换Permit。
3. Connection AUTHORIZED commit 前/后。
4. live dispatch transaction/Effect intent commit 前/后。
...
8. Call/Effect final transaction、audit/outbox、HTTP response 前后。
...
13. Confirmation preview Intent/token replay密文commit与HTTP响应前后；consume/Grant commit后、浏览器响应前。
...
22. Retry successor crash group：
    ...
    - **22a Action retry：** 分别在旧ActionAttempt/EffectDispatch终结、同一`ACQUIRED` decision及items/bucket CAS、两个successor INSERT、Call/Effect pointer、状态、audit/outbox边界pause/kill；只允许整套winner或旧current完整保留，两个旧row必须引用同一decision，Reconcile pointer保持不变。
...
- 条件G-17/G-18 reader-first、cross-version、state-machine/direct-SQL、Browser actor/isolation、crash/replay/snapshot、permanent tombstone/late-write guard和52/26/2 cross-carrier golden；Gate未关闭时CI反向证明对应active artifacts为零。
...
| A-13 OpenAPI/carrier/route golden | 枚举G-18 10+1页面variant、page-action、signed decision、terminal receipt、三分source、`PlatformTerminalAcceptanceProofV1`、proof-bearing event/snapshot/ACK、witness recovery response、两个正式restricted route和全部错误边 | 52/26/2各只在法定carrier且计数不变；route inventory按67/69与四种activation矩阵变化。冻结proof-aware wire bytes/hash、自字段排除、snapshot使用proof event identity和恢复result union；旧ACK、cross-carrier、proof字段漂移、把两route合成variant、漏任一physical instance、错identity/purpose或fallback成功化均失败 |
...
DoD：Provider前持久化；锁/fence顺序固定；跨Call/type payload、INLINE/OBJECT both/neither、并发双写和对象stage/final约束通过；ActionAttempt/Dispatch/Reconcile的nullable初始pointer、唯一predecessor/successor、current最大sequence、冻结policy/max、`N-1/N/N+1`、budget unavailable、Action pair同decision、winner/stale loser和半完成事务直接PG/并发/crash用例通过；Cancel same-key exact status/content-type/body replay、different-key fresh observation、同key异hash409及claim/Call/result/audit/outbox原子crash矩阵通过；每个可枚举crash点确定或UNCERTAIN，非幂等效果不重复；缺任一typed evidence字段、完整风险窗口、exact Catalog set或未来写RPO=0 topology时mutating全链路保持MUTATION_RECOVERY_BLOCKED，自动对账独立owner且不重放mutation。仅`ONLY_IF_G04_APPROVED` suite增加proposal/resolution direct-PG约束、maker-checker同人/篡改/过期/stale/双批准/response-loss和每个claim/insert/CAS/audit/outbox边界；same-key/hash必须回放原201/200 status/content-type/body/ETag，异hash 409且零第二transition，IAM/capability/step-up失败不得留下durable claim。当前active migration、route、carrier、edge和fixture均无Resolution能力。
...
DoD：executor无通用出网；Pod不能直连公网；双Proxy/重启同JTI至多一次request/stream，admission响应未知为零次；伪造/过期/跨Proxy proof、事务commit边界过期、SIGNER或Admission evidence/set/topology/target/manifest错配、Admission-Hop字段拼接、receipt乱序/冲突/terminal改写均零发送并在正确边界拒绝；旧cohort/错assertion/MAC/query/header/body/oldCatalog overlay或mutation gate accept为0；evidence expiry/replacement、kind切换和Catalog set变化先fence lease且从SIGNER到request-start为0；CANARY -> OPEN与OPEN -> CANARY使用新proof且不改历史；HTTP1/H2、rotate drain、key overlap、parser/fuzz、429/5xx/slow/oversize和容量通过。

ProviderFailure DoD另由25.2生成完整`purpose x effectClass x category x effectEvidence x resultViolation x retryDisposition`矩阵：每个合法组合逐字节命中唯一durable disposition，其余组合全部`ILLEGAL`；共享Provider/account budget decision必须有不可变parent/items/checksum并与successor或耗尽终态同事务，budget不可用不得乐观创建successor。七类链的second-root/sibling/cross-root、wrong scope/policy/window/revision/checksum、partial decision/item、Action pair mismatch、FORBIDDEN扣预算和stale loser反例全部通过；read-only和idempotent-write的`OUTCOME_UNKNOWN + RETRY_EXHAUSTED|RETRY_FORBIDDEN`必须分别命中25.2固定且不同的exact reason，Adapter、T-UNIT、T-PG、T-OAUTH和T-EFFECT使用同一golden source。
...
| API 契约 | 22、23、25.1.7、28.1；Browser/Admin/Internal/Adapter/Egress/Recovery/Event/Operational | 完整inventory为67 logical / 69 physical：默认build 63/65，仅G-04为65/67，仅G-17为另一operation集合的65/67，两Gate均关闭为67/69；另有独立3/12 operational。`consumeConnectionCompletion`以POST、CSRF、strict body和exact replay取代有副作用GET；G-04关闭后两个normal route各生成strict request/result/status、`NO_PUBLIC_IN_PROGRESS` carrier和7条failure edge；四build幂等矩阵固定为`29/31/30/32 HEADER_KEY + 2 DOMAIN_COMMITMENT`及`3/7/6/15\|17\|16\|18` policy tuple。G-17关闭后增加两个独立formal route：Platform `restorePlatformAlternateTerminalAcceptance`与Connection `resolvePlatformTerminalAcceptanceWitness`，各自只有一个physical instance、OpenAPI、audience、purpose和幂等模式；G-18复用二者且不新增route。52/26/2 failure literal、15/11 Resolution/NextAction及3/12不变。Definitive-loss只在无任何current path等PRD既有场景active；另一path live时G-17未批准只SAFE_DENY零写。OpenAPI baseline、physical expansion、四种build和N/N-1 evidence待WP0/WP1 |
...
| C-07 | Preauthorization先本地匹配再redeem | 18.2-18.5、19/19.1 | 21.2 Invocation/Permit + 21.5 Call write-once hash/version/association | 22.6/22.7/22.9同一commitment；introspect也绑定 | 31.2、31.6/31.7跨Invocation、不匹配、terminal replay/crash | WP6/WP8；Connection ownership/Permit ADR | 本版设计闭合；实施证据待WP6/WP8 |
...
| C-14 | logical operation、physical deployment route、error/condition/alternate response、Cancel replay与UX resolution闭环 | 22.1-22.11、25.1、28.1；67 logical / 69 physical完整inventory、四种activation与`29/31/30/32 HEADER_KEY + 2 DOMAIN_COMMITMENT` carrier矩阵、3/12 operational、pre/post-resource carrier、15个ResolutionRoute/11个NextAction | 21.2/21.4/21.5及21.6 first-submission单调门禁、immutable cancel result、completion consume exact result与zero-mutation proof；不持久化message/URL/bearer ref | 25.1.7 exact edges、alternate contract、CommandIdempotencyMode与`3/7/6/15\|17\|16\|18` policy set、physical expansion、completion POST-only/CSRF/idempotency、G-04两route各7条edge、两个G-17独立route、四入口serializer、same/different cancel key；9个internal operation逐项Identity/DB 503且durable accept后回放资源；条件route只在Gate关闭后进入active artifacts | T-OPENAPI/T-PG/T-EFFECT；52/52 + 26/26 + 2/2、三namespace两两互斥、67/69四build、carrier operation集合、15/15 ResolutionRoute、11/11 NextAction、route/Ingress/Service/NetworkPolicy/operational router双向集合、completion prefetch/refresh零写、G-04 exact status/body/quoted strong ETag、dependency pre/post-accept与Cancel bytes/multi-effect golden | WP0/WP1/WP7/WP8/WP10；API/Web/UX review，G-04/G-17各自Gate | 本版设计闭合；generated artifacts与条件build证据待对应WP |
