...
`connection-v9` ProviderRelease 只发布已由该 credential mode 真实验证的 143 个 `@v9`
...
`jenkins-ci` 固定 origin `https://jenkins-ci.agoralab.co`，复用 Jira/Confluence 已验证的公司 OAuth
password grant token provider：Connection 使用独立 OAuth client/service account取得短期 token并通过
`accessToken` header交给 HCI Auth，同时使用用户 Jenkins username + API Token Basic Auth完成 Jenkins
身份校验。该 deployment 使用独立 catalog、Credential和Grant，并进入 runtime supported providers。
...
Build artifact 有界读取与 HCI OAuth 双层鉴权发布为新的 `@v7` ActionVersion，
...
个人身份只接受同站请求携带的 HttpOnly `HCIAuthToken`；浏览器不得提交 Token 或公司密码。Connection
服务端从 HCI session 解析短期 access token，Manhattan 调用固定 user-info endpoint 验证后继续执行现有
RBAC 校验。Connection 加密保存个人 session，机器 key 只由 Secret Manager 注入，二者不得进入同一
credential envelope。手工 Bearer 版 v1 保持不可变；`manhattan-connection-v2` 发布当前用户、SDK dump 列表/详情和 Symbol
