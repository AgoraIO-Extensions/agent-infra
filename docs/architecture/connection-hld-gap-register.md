...
| OC-11：Connection 产品入口 | 部分实现；门禁未关闭 | 独立 `apps/connection-web` 已提供简体中文 LDAP 登录、Connections、访问令牌、管理员和共享 GitHub Connection React 页面；个人 Connections 页面另提供公司 Bitbucket PAT 连接/重连入口并展示 Provider。它通过生成 Browser Client 调用 `connection-api`，不属于 Agora Agent Platform Web。PAT 使用 password input、同源请求且 API 不回显。完整 Consent 历史、Call、catalog 和 audit 视图尚未冻结。 | 使用同一公司 session 完成真实管理员/普通用户桌面与移动端浏览器验收，并补齐 Consent 历史、Call、Consumer、catalog 和 audit 页面。 |
| OC-12 / WP10：Provider 适配 | 部分实现；门禁未关闭 | GitHub 通过 `openconnector-adapter` 使用固定版本 OpenConnector Kernel。Bitbucket 固定公司 Server `6.7.2`/build `6007002`、HTTPS API origin 和 Bearer PAT，使用 `whoami` + exact active user ID 证明账号；25 个 Action 通过同一五个 MCP tool 发现与执行。可重复 E2E 使用真实 PostgreSQL Credential/Grant/Application Service/ProviderRelease 链路验证 identity、project/repository/branch/PR pagination、跨 Principal/Provider拒绝、Grant revoke 和 Credential 密文；输出只有脱敏断言。真实写入、PAT rotation、限流和发布评审仍未关闭。 | 按 Provider 评审 deployment、auth、scope、identity、error、rate limit、idempotency；Bitbucket 补 PAT revoke/rotation、获批真实写操作与 crash/response-lost 证据。 |
