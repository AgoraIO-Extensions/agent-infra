...
| --- | --- | --- | --- |
| G-01：公司 LDAP 与目标 Direct MCP OAuth/PAT profile | GZ3 control plane 已部署；广泛生产验收未关闭 | 统一账号 ADR 已批准；正式 runtime 已装配 Connection OAuth、LDAP browser session、Portable PAT、LDAP Adapter、PostgreSQL session/token authority 和 Direct MCP。控制台先登录再签发 PAT，不重复收集 LDAP 密码；browser session 与 PAT 明文均不落库，PAT 复用 Principal freshness 与统一 Bearer verifier。当前 Agora LDAP profile 明确使用公司私网 `ldap://` direct bind，不存在自动 downgrade/fallback；Connection Owner 已在 #601 接受 GZ3 control plane 与当前 active-attribute 缺口，仅批准 `agent-connector.gz3.agoralab.co` 与已验证 Codex OAuth 客户端。生产与 conformance 使用同一 Runtime factory；其他环境和客户端仍 fail closed。 | 在 GZ3 完成真实 browser session、OAuth metadata、受限 DCR、PKCE S256、resource、refresh rotation、实例撤销和 PAT Bearer 调用；补齐账号禁用与私网边界证据后，再由 Identity/Security 决定广泛支持。 |
