...
| OC-11：Connection 产品入口 | 部分实现；门禁未关闭 | 独立 `apps/connection-web` 已提供简体中文 LDAP 登录、Connections、访问令牌、管理员和共享 GitHub Connection React 页面；它通过生成 Browser Client 调用 `connection-api`，不属于 Agora Agent Platform Web。服务端 RBAC保护管理 API。普通用户可连接多个个人 GitHub账号、查看 eligible shared账号、预览并确认 Consumer 换号、revoke Consumer、disconnect个人 Connection 和独立撤销 PAT；管理员可创建和改名 SharedScope、管理显式 membership且不能读取 Credential 明文。完整 Consent 历史、Call、catalog 和 audit 视图尚未冻结。 | 使用同一公司 session 完成真实管理员/普通用户桌面与移动端浏览器验收，并补齐 Consent 历史、Call、Consumer、catalog 和 audit 页面。 |
