...
| OC-05：账号与 Credential 生命周期 | 部分实现；门禁未关闭 | PostgreSQL Repository 已实现 GitHub OAuth transaction、Bitbucket PAT、Jira Server Basic 用户名/密码 credential 与服务端短期 accessToken provider、账号连接/断开、AES-256-GCM Credential 加密与仅在 Adapter 执行边界解密注入；同一 Principal 可保存多个 Provider Connection，并通过短期授权预览为每个 Direct Consumer 选择唯一 current Connection。当前使用进程注入的单一加密 key，不是 KMS envelope key；Jira Token Server refresh/rotation、stable account proof 和持久 attempt 尚未实现。 | KMS 支持的 Credential 生命周期、Token Server refresh/rotation/revoke、稳定 Provider 身份、持久 attempt，以及真实账号的换号、重连和 scope 变化测试。 |
...
| OC-09：KMS 与受控 egress | 未关闭 | 正式 runtime 使用进程注入的 AES-256-GCM key；Bitbucket 和 Jira Server Adapter 使用受控 guarded fetch，对固定 origin 执行 DNS/IP、private-network、redirect 和响应大小/timeout 校验。尚无 KMS envelope key、唯一 egress proxy/连接级 IP pinning 或 secret canary；生产镜像继续不发布业务路由。 | KMS envelope key 与唯一 egress proxy、DNS rebinding/IP pinning、secret canary 证据。 |
...
| OC-11：Connection 产品入口 | 部分实现；门禁未关闭 | 独立 `apps/connection-web` 已提供简体中文 LDAP 登录、Connections、访问令牌、管理员和共享 GitHub Connection React 页面；个人 Connections 页面另提供公司 Bitbucket PAT 和 Jira Server credential 连接/重连入口并展示 Provider。它通过生成 Browser Client 调用 `connection-api`，不属于 Agora Agent Platform Web。Provider credential 使用 password input、同源请求且 API 不回显。完整 Consent 历史、Call、catalog 和 audit 视图尚未冻结。 | 使用同一公司 session 完成真实管理员/普通用户桌面与移动端浏览器验收，并补齐 Consent 历史、Call、Consumer、catalog 和 audit 页面。 |
| OC-12 / WP10：Provider 适配 | 部分实现；门禁未关闭 | GitHub 通过 `openconnector-adapter` 使用固定版本 OpenConnector Kernel。Bitbucket 固定公司 Server `6.7.2`/build `6007002`、HTTPS API origin 和 Bearer PAT，使用 `whoami` + exact active user ID 证明账号；25 个 Action 通过同一五个 MCP tool 发现与执行。Jira 固定公司 Server `7.11.0`/build `711000`、REST v2、每用户 Basic credential 加服务端短期 accessToken provider，使用 `myself` + active identity proof；20 个 Action 通过同一五个 MCP tool 发现与执行。可重复 E2E 使用真实 PostgreSQL Credential/Grant/Application Service/ProviderRelease 链路验证 identity、项目/Issue/评论、跨 Principal/Provider 拒绝、Grant revoke 和 Credential 密文；输出只有脱敏断言。真实 Jira 写入、Token Server rotation、限流和发布评审仍未关闭。 | 按 Provider 评审 deployment、auth、scope、identity、error、rate limit、idempotency；Bitbucket/Jira 补 PAT 或服务端 accessToken revoke/rotation、获批真实写操作与 crash/response-lost 证据。 |
