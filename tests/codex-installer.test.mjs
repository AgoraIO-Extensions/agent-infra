...
test("upstream installer produces the verifier's immutable file modes", async () => {
	...
	try {
		...
		await copyFile(new URL("../deploy/runtime/install-codex.mjs", import.meta.url), installer);
		...
		const manifest = {
			...
			artifacts: { amd64: { name, archiveSha256: hash(await readFile(archive)), executableSha256: hash(binary) } },
			legal: { LICENSE: hash("synthetic license") },
		};
		await writeFile(join(root, "packages/agent-runtime/src/codex-release.json"), JSON.stringify(manifest));
...
globalThis.fetch = async (url) => new Response(url.endsWith(".tar.gz") ? await readFile(new URL("./archive.tar.gz", import.meta.url)) : "synthetic license");`);
		execFileSync(process.execPath, ["--import", preload, installer, "amd64", destination]);
		for (const [path, mode] of [
			["", 0o555], ["bin", 0o555], ["share", 0o555],
			["bin/codex", 0o555], ["share/release.json", 0o444], ["share/LICENSE", 0o444],
		]) assert.equal((await stat(join(destination, path))).mode & 0o7777, mode, path);
		assert.deepEqual(await readFile(join(destination, "bin/codex")), binary);
	} finally {
		for (const path of [destination, join(destination, "bin"), join(destination, "share")])
